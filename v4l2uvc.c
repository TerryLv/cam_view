...
static int v4l2uvc_init_v4l2(struct vdIn *vd);
...
int v4l2uvc_check_videoIn(struct vdIn *vd, char *device)
{
	int ret;
...
int v4l2uvc_init_videoIn(struct vdIn *vd, char *device, int width, int height, float fps,
...
int v4l2uvc_enum_controls(int vd) //struct vdIn *vd)
{    
...
int load_controls(int vd) //struct vdIn *vd)
{
  ...
  configfile = fopen("camview.cfg", "r");
  if ( configfile == NULL) {
    perror("configfile camview.cfg open failed");
  }
  else {
    printf("loading controls from camview.cfg\n");
    char buffer[512]; 
...
int v4l2uvc_capture(struct vdIn *vd)
{
...
int v4l2uvc_close_v4l2(struct vdIn *vd)
{
    if (vd->isstreaming)
	    video_disable(vd);
    if (vd->tmpbuffer)
